...
def test_set_parameters():
    ...
    assert jnp.allclose(cond_llkh.mu, jnp.ones((4,)) * 2)
    assert jnp.allclose(cond_llkh.sigma, jnp.ones((4,)))

    cond_llkh = cond_llkh.set_params(
        GaussianIidParameter(mu=4 * jnp.ones((4,)), sigma=3 * jnp.ones((4,)))
    )
    assert jnp.allclose(cond_llkh.mu, jnp.ones((4,)) * 4)
    assert jnp.allclose(cond_llkh.sigma, jnp.ones((4,)) * 3)

...
class GUMSampler(eqx.Module):
    ...
    method: Literal["spectral", "fourier"] = eqx.field(
        static=True, kw_only=True, default="spectral"
    )
    ...
    def sample_image(self, model: GUM, key: Key) -> tuple[Float[Array, "lx ly"], Array]:
        ...
        gmrf = GMRF(
            kappa=model.kappa, nu=model.nu, r=model.r, dim=2
        )  # dim is fixed since we want to
        if self.method == "spectral":
            ...
            gmrf_sampler = SpectralSamplerGMRF(
                n_bands=self.n_bands, lx=self.lx, ly=self.ly
            )
        ...
        z = jnp.stack(
            [gmrf_sampler.sample_image(gmrf, subkeys[k]) for k in range(model.K - 1)],
            axis=0,
        )
        ...
        dists = jnp.stack(
            [
                jnp.linalg.norm(z_fl - vertices[v].reshape((-1, 1)), axis=0)
